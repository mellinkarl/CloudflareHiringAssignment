...
export async function onRequestPost(context) {
    ...
    for (let i = 0; i < requestNotifications.length; i++) {
        if (
            !["alert", "info", "success"].includes(
                requestNotifications[i].type,
            ) ||
    ...
    try {
        let notifications = await context.env.cloudflareHiringAssignmentKV.get(
            "notifications",
            "json",
        );
        ...
        await context.env.cloudflareHiringAssignmentKV.put(
            "notifications",
...
export async function onRequestDelete(context) {
    ...
    return new Response(
        JSON.stringify({ message: "Notifications deleted successfully!" }),
        {
            headers: { "Content-Type": "application/json" },
        },
    );
